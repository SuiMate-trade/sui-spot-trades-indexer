...
export async function pushToSQS(queueName, content) {
    try {
        ...
        const result = await client
            ...
            .promise();
        log(`Successfully published event to SQS. Message ID: ${result.MessageId}`, LogLevel.DEBUG);
        return result;
    }
    catch (err) {
...
async function handleMessage(handlerFunc, body) {
    try {
        const result = await handlerFunc(JSON.parse(body));
        log(`Handling of SQS event completed with result ${JSON.stringify(result)}`, LogLevel.INFO);
        return true;
    }
    catch (error) {
        log(`Error was found during working on model ${body}\n${error.stack}`, LogLevel.DEBUG);
        return false;
    }
}
export async function handleSqsEvent(handlerFunc, event) {
